...
#pragma mark - 新增，下载image后不显示出来（首页大屏广告用）

-(void)setImageWithURLNotShowAfterDownload:(NSURL *)url
{
    [self setImageWithURLNotShowAfterDownload:url placeholderImage:nil options:0 progress:nil completed:nil dealedBlock:nil];
}
- (void)setImageWithURLNotShowAfterDownload:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletedBlock)completedBlock dealedBlock:(MJWebImageDealedBlock)dealedBlock
{
    [self cancelCurrentImageLoad];
